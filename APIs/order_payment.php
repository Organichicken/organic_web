...
if(sqlValue("SELECT COUNT(*) FROM `employee_otp_key` WHERE `nkey` = '".$user_hash_key."' AND `user_phone` = '".$user_phone."'") && $user_id = sqlValue("SELECT `user_id` FROM `users` WHERE `phone` = '".$user_phone."'")){
    ...
    if(!empty($temp_order['cashback'])){
        db_query("INSERT INTO `user_wallet`(`user_id`, `amount`, `transaction_id`, `description`, `status`, `updated_at`) VALUES ('".db_escape($user_id)."','+".db_escape($temp_order['cashback'])."','','Cash back received from order ".$order_key."','success','".$now."')");
    }
    if(!empty($temp_order['user_wallet'])){
        db_query("INSERT INTO `user_wallet`(`user_id`, `amount`, `transaction_id`, `description`, `status`, `updated_at`) VALUES ('".db_escape($user_id)."','-".$temp_order['user_wallet']."','','Used wallet amount on order ".$order_key."','success','".$now."')");
    }

    //Get order data for given order id
    if(db_query("INSERT INTO `payments`(`user_id`, `order_id`, `amount`, `razorpay_order_id`, `razorpay_signature`, `razorpay_payment_id`, `mode`, `status`, `timestamp`) VALUES ('".$user_id."','".$order_key."','".makesafe($_POST['amount'])."','".makesafe($_POST['razorpay_order_id'])."','".makesafe($_POST['razorpay_signature'])."','".makesafe($_POST['razorpay_payment_id'])."','".makesafe($_POST['mode'])."','".makesafe($_POST['status'])."','".date('Y-m-d H:i:s')."')")){
