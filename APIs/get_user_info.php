...
if(sqlValue("SELECT COUNT(*) FROM `employee_otp_key` WHERE `nkey` = '".$user_hash_key."' AND `user_phone` = '".$user_phone."'") && $user_id = sqlValue("SELECT `user_id` FROM `users` WHERE `phone` = '".$user_phone."'")){
    ...
    $data['user_address'] = sqlr("SELECT `user_id`, `address_id`, `name`, `phone`, `alternative_phone`, `house_no`, `building_name`, `street`, `landmark`, `pincode`, `address`, `latitude`, `longitude`, `city`, `address_type`, `is_default_address` FROM `address` WHERE user_id = '".$user_id."' AND is_default_address = '1' LIMIT 1");
