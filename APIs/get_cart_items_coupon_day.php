...
if(sqlValue("SELECT COUNT(*) FROM `employee_otp_key` WHERE `nkey` = '".$user_hash_key."' AND `user_phone` = '".$user_phone."'") && $user_id = sqlValue("SELECT `user_id` FROM `users` WHERE `phone` = '".$user_phone."'")){
    ...
    foreach ($user_cart_data as $row) {
		...
		if($row['item_id'] == '#OCITM_MO5KL') $sp_item++;
    }
    ...
    if($user_cart_data){
        ...
        if(!empty($_POST['offer_id'])){
            ...
            if($offer['status'] == "1"){
				if($offer['offer_id'] == '#OCOFR_wnQAA'){
					if($sp_item > 0){
						if(coupon_per_day_count($offer['offer_id'],date('Y-m-d')) >= 5){
							echo json_encode(array('status'=>404,'message'=>'Coupon limit reached for this day'));
							exit;
						}
					}
					else{
						echo json_encode(array('status'=>404,'message'=>'Offer wont apply on these item(s)'));
						exit;
					}
				}
